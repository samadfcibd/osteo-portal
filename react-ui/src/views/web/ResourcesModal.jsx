...
const ResourcesModal = ({
    ...
    const getOrganismRating = async (organismId) => {
        try {
            const response = await axios.get(`${process.env.VITE_API_URL}/api/organisms/${organismId}/rating`);
    ...
    const fetchOrganismList = async (page = 1) => {
        ...
        try {
            const response = await axios.get(`${process.env.REACT_APP_BACKEND_SERVER}api/organisms`, {
                params: {
                    stage: selectedStage,
                    page: page,
