...
const ProfileSection = () => {
    ...
    const handleLogout = () => {
        ...
        // Check if token exists and is still valid (client-side)
        if (!account.token || isTokenExpired(account.token)) {
            // Token is invalid/expired; force cleanup
    ...
    // Helper function to check JWT expiry client-side
            ...
            try {
                ...
                if (expiresIn > 0) {
                    const timer = setTimeout(() => {
                        // Trigger logout 1 minute before actual expiration
                        dispatcher({ type: LOGOUT });
                    }, expiresIn - 60000); // 60000ms = 1 minute buffer
